...
def clickBtn(img,name=None, timeout=3, threshold = ct['default']):
    ...
    while(not clicked):
        ...
        moveToWithRandomness(pos_click_x,pos_click_y,0.2)
        pyautogui.click()
        return True
...
def clickButtons():
    ...
    for (x, y, w, h) in buttons:
        moveToWithRandomness(x+(w/2),y+(h/2),0.2)
        pyautogui.click()
        global hero_clicks
...
def clickGreenBarButtons():
    ...
    for (x, y, w, h) in not_working_green_bars:
        # isWorking(y, buttons)
        moveToWithRandomness(x+offset+(w/2),y+(h/2),0.2)
        pyautogui.click()
        global hero_clicks
        hero_clicks = hero_clicks + 1
        if hero_clicks > 20:
    ...
    for position in heroes_positions:
        if not isHome(position,go_home_buttons):
            ...
            if(not isWorking(position, go_work_buttons)):
                ...
                moveToWithRandomness(go_home_buttons[0][0]+go_home_buttons[0][2]/2,position[1]+position[3]/2,0.2)
                pyautogui.click()
            else:
